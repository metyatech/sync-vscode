...
export async function showSftpError(error: unknown, fallbackPrefix?: string): Promise<boolean> {
    ...
    } else if (errMsg.includes('No such user') || errMsg.includes('All configured authentication methods failed')) {
        showError(ErrorCode.AuthFailed, fallbackPrefix);
        return await promptAuthReentry();
    } else if (errMsg.includes('Timed out') || errMsg.includes('timeout') || errMsg.includes('handshake')) {
