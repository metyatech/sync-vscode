...
import {
    sftpMkdirRecursive,
    listRemoteFilesRecursiveRelative,
    listLocalFilesRecursiveRelative,
    handleDelete,
    sftpRmdirRecursive,
    SftpListError,
} from './sftpUtils';
                ...
                try {
                    ...
                    if (stat.size > config.maxUploadSize) {
                        vscode.window.showWarningMessage(
                            `「${rel}」はファイルサイズが上限(${(config.maxUploadSize / 1024 / 1024).toFixed(1)}MB)を超えているため送信しません。`,
                        );
    ...
    try {
        ...
        while (!initSuccess) {
            try {
                ...
                initSuccess = true;
                console.log(
                    `リモートパス初期化とファイルリスト取得が成功しました。${remotePaths.length}アイテム検出。`,
                );
            } catch (error) {
                if (error instanceof SftpListError) {
                    console.warn(
                        `リモートパスの初期化またはリスト取得中にエラー発生 (Path: ${error.path}, PermissionError: ${error.isPermissionError}): ${error.message}`,
                    );

                    // SftpListError の場合、ユーザーに再入力を促す
                    const settingsUpdated = await showSftpError(
                        error,
        ...
        for (const rel of both) {
            ...
            try {
                statLocal = fs.statSync(localFull);
            } catch {
                continue;
            }
            if (!statLocal.isFile()) {
